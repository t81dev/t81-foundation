...
* **No silent behavioral drift**
  ...
  * If you change observable behavior of an existing public function, you MUST:
    ...
    2. Update the relevant spec(s) in `spec/`.
    3. Document the change in comments or changelog as appropriate.
...
2. Locate the corresponding spec sections in `spec/` (TISC, T81VM, data types, etc.).
3. Design an appropriate C++ API in `include/t81/`:

   * Keep it small, composable, and consistent with existing types.
4. Implement the API in `src/`, reusing logic from the CWEB implementation where helpful.
5. Add or extend tests to prove equivalence with the legacy behavior.
6. Leave the legacy CWEB code intact; add comments noting the existence of a modern replacement.
...
2. Add comments explaining assumptions.
