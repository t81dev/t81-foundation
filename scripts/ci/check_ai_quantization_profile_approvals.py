#!/usr/bin/env python3
"""Validate approval metadata for quantization codec profile history windows."""
...
def main() -> int:
    ...
    for idx, win in enumerate(windows, start=1):
        ...
        if not isinstance(profile, dict):
            errors.append(f"{window_id}: profile must be object")
            profile = {}
    ...
    print(f"ai quantization profile approvals status: {status}")
