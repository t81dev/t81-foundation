...
- RFC-0026 WLOAD policy-evidence readiness tracker gate (requires allow-path evidence while retaining deny-path coverage): `check_ai_wload_policy_evidence.py`
...
# Strict-deterministic matrix probes are expected to resolve through the governed `t81_reference_vm` reference backend; host-float backends remain non-strict.
python3 scripts/ci/check_ai_benchmark_capability_matrix.py --out-dir build/ai-benchmark --ai-bin build/experiments/ai/ux_tools/t81_ai --model-file tests/fixtures/llama_cpp_repro/model.gguf --formats gguf,t3k --modes strict_deterministic --expectations-file scripts/ci/ai_benchmark_capability_expectations.json
...
python3 scripts/ci/check_ai_inference_capability_matrix.py --out-dir build/ai-ux --ai-bin build/experiments/ai/ux_tools/t81_ai --model-file tests/fixtures/llama_cpp_repro/model.gguf --formats gguf,t3k --modes strict_deterministic --expectations-file scripts/ci/ai_inference_capability_expectations.json
