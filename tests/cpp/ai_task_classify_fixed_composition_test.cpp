...
CommandResult run_cli(const fs::path& bin_path, const std::vector<std::string>& args,
  ...
  const fs::path out_path = make_temp_path("t81-classify-compose", ".out");
  const fs::path err_path = make_temp_path("t81-classify-compose", ".err");
...
int main(int argc, char* argv[]) {
  ...
  const fs::path tmp_root = make_temp_path("t81-classify-compose", "");
  ...
  T81_TEST_CHECK(
      contains(result_artifact.stdout_text, "\"schema\": \"t81.ai.task.classify-fixed.v1\""));
  T81_TEST_CHECK(contains(result_artifact.stdout_text, "\"label\": \"POSITIVE\""));
  T81_TEST_CHECK(contains(result_artifact.stdout_text, "\"termination_reason\": "));

  const auto typed_label = run_cli(t81_bin, {"ai", "task", "read-field", *result_ref, "--field",
                                             "label", "--canonfs-root", canonfs_root.string()});
  const auto typed_termination =
      run_cli(t81_bin, {"ai", "task", "read-field", *result_ref, "--field", "termination_reason",
                        "--canonfs-root", canonfs_root.string()});
  T81_TEST_CHECK(typed_label.exit_code == 0);
  T81_TEST_CHECK(typed_termination.exit_code == 0);
  T81_TEST_CHECK(typed_label.stdout_text == "POSITIVE\n");
  T81_TEST_CHECK(typed_termination.stdout_text == "single_step_max_score\n");

  const fs::path rule_set_path = ruleset_dir / "positive.ruleset";
  {
  ...
  const auto action_ref = run_cli(t81_bin, {"canonfs", "put-file", rule_set_path.string(),
                                            "--canonfs-root", canonfs_root.string()});
  ...
  const auto cross_root_action_ref =
      run_cli(t81_bin, {"canonfs", "put-file", rule_set_path_alt.string(), "--canonfs-root",
  ...
  const auto repeat_bundle_store_result = run_cli(
      t81_bin, {"artifact", "store-bundle", "--schema", "t81.ai.task.classify-fixed.bundle.v1",
                "--field", "source_result_ref=" + *result_ref, "--field",
                "source_provenance_ref=" + *provenance_ref, "--field", "action_ref=" + action_hash,
                "--field", "record_ref=" + *record_ref, "--canonfs-root", canonfs_root.string()});
