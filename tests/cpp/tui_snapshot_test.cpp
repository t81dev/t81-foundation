...
static void test_command_palette() {
  ...
  const std::vector<std::string> required_names = {
      "code build", "code run", "axion status", "trace show", "canonfs list", "weights verify",
  };
...
static void test_workspace_state_roundtrip() {
  ...
  original.current_target = {t81::tui::TargetKind::SourceFile, "examples/bigint_demo.t81",
                             "bigint_demo.t81", ""};
  original.current_source = original.current_target;
  original.current_trace = {t81::tui::TargetKind::TraceFile, "runs/bigint.trace", "bigint.trace",
                            ""};
...
static void test_target_recency_ordering() {
  ...
  t81::tui::remember_target(recent, current_target, current_source, current_artifact, current_trace,
                            current_policy, a, 3);
  t81::tui::remember_target(recent, current_target, current_source, current_artifact, current_trace,
                            current_policy, b, 3);
  t81::tui::remember_target(recent, current_target, current_source, current_artifact, current_trace,
...
static void test_palette_clamps_stale_selection() {
  ...
  palette_selected = std::max(0, std::min(palette_selected, static_cast<int>(filtered.size()) - 1));
...
static void test_tui_helpers() {
  ...
  T81_TEST_CHECK(t81::tui::scroll_indicator_text(10, 37, 10, " ↑↓") == " L11-20/37 ↑↓");
  T81_TEST_CHECK(t81::tui::scroll_indicator_text(0, 0, 22, " ↑↓") == " L0-0/0 ↑↓");
  T81_TEST_CHECK(t81::tui::scroll_indicator_text(999, 37, 10, " ↑↓/PgUp/PgDn ") ==
                 " L28-37/37 ↑↓/PgUp/PgDn ");
