...
int main() {
    ...
    program.complex_pool.push_back(T81Complex<18>(
        T81Complex<18>::FloatType::from_double(1.5), 
        T81Complex<18>::FloatType::from_double(-2.5)
    ));
    
    program.complex_pool.push_back(T81Complex<18>(
        T81Complex<18>::FloatType::from_double(-3.14159), 
    ...
    for (int run = 0; run < 3; ++run) {
        ...
        for (size_t i = 0; i < program.complex_pool.size(); ++i) {
            ...
            std::cout << "Run " << run + 1 << ", Complex " << i 
                      << ": (" << orig_real << ", " << orig_imag << "i) ✓" << std::endl;
        }
    }
