...
CommandResult run_cli(const fs::path& bin_path, const std::vector<std::string>& args,
  ...
  const fs::path out_path = make_temp_path("t81-assess-fixed-test", ".out");
  const fs::path err_path = make_temp_path("t81-assess-fixed-test", ".err");
...
void write_assess_fixed_demo_model(const fs::path& out_dir) {
  std::error_code ec;
  fs::create_directories(out_dir, ec);
  T81_TEST_CHECK(!ec);

  const fs::path model_path = out_dir / "assess-fixed-demo.t81w";
...
int main(int argc, char* argv[]) {
  ...
  const fs::path model_dir = make_temp_path("t81-assess-fixed-model-dir", "");
  write_assess_fixed_demo_model(model_dir);
  const fs::path model_path = model_dir / "assess-fixed-demo.t81w";
  const fs::path canonfs_root = make_temp_path("t81-assess-fixed-canonfs", "");
  ...
  const fs::path allow_policy = make_temp_path("t81-assess-fixed-allow", ".apl");
  {
  ...
  const fs::path deny_policy = make_temp_path("t81-assess-fixed-deny", ".apl");
  {
  ...
  const fs::path missing_marker_policy = make_temp_path("t81-assess-fixed-missing-marker", ".apl");
  {
  ...
  const fs::path unsupported_model_path = make_temp_path("t81-assess-fixed-unsupported", ".t81w");
  {
  ...
  const fs::path unsupported_policy = make_temp_path("t81-assess-fixed-unsupported", ".apl");
  {
  ...
  const std::vector<std::string> base_args = {
      "ai",          "task",        "assess-fixed", "--model",      "assess-fixed-demo",
  ...
  T81_TEST_CHECK(contains(missing_policy_result.stderr_text,
                          "error: assess-fixed requires --policy <file>"));
  ...
  T81_TEST_CHECK(contains(non_strict_result.stderr_text,
                          "error: assess-fixed requires --mode strict_deterministic"));

  const auto unsupported_result =
      run_cli(t81_bin, {"ai", "task", "assess-fixed", "--model", "unsupported-demo",
                        "--model-file", unsupported_model_path.string(), "--policy",
                        unsupported_policy.string(), "--canonfs-root", canonfs_root.string(),
  ...
  T81_TEST_CHECK(contains(
      ...
      "error: assess-fixed supports only llama-dense-v1 narrow greedy decode"));

  auto allow_args = base_args;
  allow_args.push_back("--input");
  allow_args.push_back("greet hello");

  const auto first = run_cli(t81_bin, allow_args);
  const auto second = run_cli(t81_bin, allow_args);
  ...
  T81_TEST_CHECK(contains(first.stdout_text, "\"decision\": \"ALLOW\""));
  T81_TEST_CHECK(contains(first.stdout_text, "\"reason_code\": \"GREETING_PAIR\""));
  T81_TEST_CHECK(contains(first.stdout_text, "\"decision_token_ids\": [42]"));
  ...
  T81_TEST_CHECK(contains(first_result_artifact.stdout_text,
                          "\"schema\": \"t81.ai.task.assess-fixed.v1\""));
  T81_TEST_CHECK(contains(first_result_artifact.stdout_text, "\"decision\": \"ALLOW\""));
  T81_TEST_CHECK(contains(first_result_artifact.stdout_text, "\"reason_code\": \"GREETING_PAIR\""));
  T81_TEST_CHECK(contains(first_result_artifact.stdout_text, "\"decision_token_ids\": [42]"));

  auto deny_args = base_args;
  deny_args.push_back("--input");
  deny_args.push_back("hello world");
  const auto deny_decision_result = run_cli(t81_bin, deny_args);
  T81_TEST_CHECK(deny_decision_result.exit_code == 0);
  T81_TEST_CHECK(contains(deny_decision_result.stdout_text, "\"decision\": \"DENY\""));
  T81_TEST_CHECK(contains(deny_decision_result.stdout_text, "\"reason_code\": \"WORLD_SEQUENCE\""));
  T81_TEST_CHECK(contains(deny_decision_result.stdout_text, "\"decision_token_ids\": [9]"));

  auto review_args = base_args;
  review_args.push_back("--input");
  review_args.push_back("greet");
  const auto review_result = run_cli(t81_bin, review_args);
  T81_TEST_CHECK(review_result.exit_code == 0);
  T81_TEST_CHECK(contains(review_result.stdout_text, "\"decision\": \"REVIEW\""));
  T81_TEST_CHECK(contains(review_result.stdout_text, "\"reason_code\": \"PREFIX_ONLY\""));
  T81_TEST_CHECK(contains(review_result.stdout_text, "\"decision_token_ids\": [7]"));
  ...
  const auto denied_result =
      run_cli(t81_bin, {"ai", "task", "assess-fixed", "--model", "assess-fixed-demo",
                        ...
                        "--canonfs-root", canonfs_root.string(), "--mode",
                        "strict_deterministic", "--input", "greet hello"});
  T81_TEST_CHECK(denied_result.exit_code == 13);
  T81_TEST_CHECK(contains(denied_result.stderr_text, "SecurityFault"));
  T81_TEST_CHECK(contains(denied_result.stderr_text,
