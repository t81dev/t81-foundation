...
void test_map_determinism() {
    // Test with T81Symbol keys (should use perfect hash)
    {
        ...
        t81::T81Symbol s1("beta");
        t81::T81Symbol s2("alpha"); 
        t81::T81Symbol s3("gamma");
    ...
    // Test with string keys (should use CanonHash)
    {
        ...
        t81::T81String s1("zebra");
        t81::T81String s2("apple");
        t81::T81String s3("banana");
    ...
    // Test with integer keys
    {
        ...
        int_map[42] = t81::T81String("answer");
        int_map[7] = t81::T81String("lucky");
        int_map[3] = t81::T81String("pi");
        int_map[100] = t81::T81String("century");
    ...
    // Test deterministic vs non-deterministic iterator consistency
    {
        ...
        map[t81::T81String("x")] = 1;
        map[t81::T81String("y")] = 2;
        map[t81::T81String("z")] = 3;
    ...
    // Test single element map determinism
    {
        ...
        single_map[t81::T81String("only")] = 42;
        
        auto items = std::vector<std::pair<t81::T81String, int>>(
            single_map.dbegin(), single_map.dend()
        );
        assert(items.size() == 1);
        assert(items[0].first == t81::T81String("only"));
...
void test_map_hash_robustness() {
    ...
    t81::T81String keys[] = {"omega", "alpha", "beta", "gamma", "delta"};
