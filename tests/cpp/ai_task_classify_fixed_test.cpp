...
CommandResult run_cli(const fs::path& bin_path, const std::vector<std::string>& args,
  ...
  const fs::path out_path = make_temp_path("t81-classify-fixed-test", ".out");
  const fs::path err_path = make_temp_path("t81-classify-fixed-test", ".err");
...
void write_classify_fixed_demo_model(const fs::path& out_dir) {
  std::error_code ec;
  fs::create_directories(out_dir, ec);
  T81_TEST_CHECK(!ec);

  const fs::path model_path = out_dir / "classify-fixed-demo.t81w";
...
int main(int argc, char* argv[]) {
  ...
  const fs::path model_dir = make_temp_path("t81-classify-fixed-model-dir", "");
  write_classify_fixed_demo_model(model_dir);
  const fs::path model_path = model_dir / "classify-fixed-demo.t81w";
  const fs::path canonfs_root = make_temp_path("t81-classify-fixed-canonfs", "");
  ...
  const fs::path allow_policy = make_temp_path("t81-classify-fixed-allow", ".apl");
  {
  ...
  const fs::path deny_policy = make_temp_path("t81-classify-fixed-deny", ".apl");
  {
  ...
  const fs::path missing_marker_policy = make_temp_path("t81-classify-fixed-missing-marker", ".apl");
  {
  ...
  const fs::path unsupported_model_path = make_temp_path("t81-classify-fixed-unsupported", ".t81w");
  {
  ...
  const fs::path unsupported_policy = make_temp_path("t81-classify-fixed-unsupported", ".apl");
  {
  ...
  const std::vector<std::string> base_args = {
      "ai",          "task",        "classify-fixed", "--model",      "classify-fixed-demo",
      "--model-file", model_path.string(), "--policy", allow_policy.string(), "--canonfs-root",
      canonfs_root.string(), "--mode", "strict_deterministic"};

  auto positive_args = base_args;
  positive_args.push_back("--input");
  positive_args.push_back("greet hello");
  ...
  T81_TEST_CHECK(contains(missing_policy_result.stderr_text,
                          "error: classify-fixed requires --policy <file>"));
  ...
  T81_TEST_CHECK(contains(non_strict_result.stderr_text,
                          "error: classify-fixed requires --mode strict_deterministic"));

  const auto unsupported_result =
      run_cli(t81_bin, {"ai", "task", "classify-fixed", "--model", "unsupported-demo",
  ...
  T81_TEST_CHECK(contains(
      ...
      "error: classify-fixed supports only llama-dense-v1 narrow greedy decode"));

  const auto first = run_cli(t81_bin, positive_args);
  const auto second = run_cli(t81_bin, positive_args);
  ...
  T81_TEST_CHECK(contains(first.stdout_text, "\"label\": \"POSITIVE\""));
  T81_TEST_CHECK(contains(first.stdout_text, "\"label_token_ids\": [42]"));
  ...
  T81_TEST_CHECK(contains(first_result_artifact.stdout_text,
                          "\"schema\": \"t81.ai.task.classify-fixed.v1\""));
  T81_TEST_CHECK(contains(first_result_artifact.stdout_text, "\"label\": \"POSITIVE\""));
  T81_TEST_CHECK(contains(first_result_artifact.stdout_text, "\"label_token_ids\": [42]"));

  auto negative_args = base_args;
  negative_args.push_back("--input");
  negative_args.push_back("hello world");
  const auto negative_result = run_cli(t81_bin, negative_args);
  T81_TEST_CHECK(negative_result.exit_code == 0);
  T81_TEST_CHECK(contains(negative_result.stdout_text, "\"label\": \"NEGATIVE\""));
  T81_TEST_CHECK(contains(negative_result.stdout_text, "\"label_token_ids\": [9]"));
