...
CommandResult run_cli(const fs::path& bin_path, const std::vector<std::string>& args,
  ...
  const fs::path out_path = make_temp_path("t81-route-fixed-test", ".out");
  const fs::path err_path = make_temp_path("t81-route-fixed-test", ".err");
...
void write_route_fixed_demo_model(const fs::path& out_dir) {
  std::error_code ec;
  fs::create_directories(out_dir, ec);
  T81_TEST_CHECK(!ec);

  const fs::path model_path = out_dir / "route-fixed-demo.t81w";
...
int main(int argc, char* argv[]) {
  ...
  const fs::path model_dir = make_temp_path("t81-route-fixed-model-dir", "");
  const fs::path route_builder = t81_bin.parent_path() / "t81_make_route_fixed_demo";
  if (fs::exists(route_builder)) {
    const auto builder_result = run_cli(route_builder, {model_dir.string()});
    T81_TEST_CHECK(builder_result.exit_code == 0);
  } else {
    write_route_fixed_demo_model(model_dir);
  }
  const fs::path model_path = model_dir / "route-fixed-demo.t81w";
  const fs::path canonfs_root = make_temp_path("t81-route-fixed-canonfs", "");
  ...
  const fs::path allow_policy = make_temp_path("t81-route-fixed-allow", ".apl");
  {
  ...
  const fs::path deny_policy = make_temp_path("t81-route-fixed-deny", ".apl");
  {
  ...
  const fs::path missing_marker_policy = make_temp_path("t81-route-fixed-missing-marker", ".apl");
  {
  ...
  const fs::path unsupported_model_path = make_temp_path("t81-route-fixed-unsupported", ".t81w");
  {
  ...
  const fs::path unsupported_policy = make_temp_path("t81-route-fixed-unsupported", ".apl");
  {
  ...
  const std::vector<std::string> base_args = {
      "ai",          "task",        "route-fixed", "--model",      "route-fixed-demo",
  ...
  const auto missing_policy_result =
      run_cli(t81_bin, {"ai", "task", "route-fixed", "--model", "route-fixed-demo",
  ...
  T81_TEST_CHECK(contains(missing_policy_result.stderr_text,
                          "error: route-fixed requires --policy <file>"));
  ...
  T81_TEST_CHECK(contains(non_strict_result.stderr_text,
                          "error: route-fixed requires --mode strict_deterministic"));

  const auto unsupported_result =
      run_cli(t81_bin, {"ai", "task", "route-fixed", "--model", "unsupported-demo",
  ...
  T81_TEST_CHECK(contains(
      ...
      "error: route-fixed supports only llama-dense-v1 narrow greedy decode"));

  auto route_a_args = base_args;
  route_a_args.push_back("--input");
  route_a_args.push_back("greet hello");

  const auto first = run_cli(t81_bin, route_a_args);
  const auto second = run_cli(t81_bin, route_a_args);
  ...
  T81_TEST_CHECK(contains(first.stdout_text, "\"route\": \"A\""));
  T81_TEST_CHECK(contains(first.stdout_text, "\"route_token_ids\": [42]"));
  ...
  T81_TEST_CHECK(contains(first_result_artifact.stdout_text,
                          "\"schema\": \"t81.ai.task.route-fixed.v1\""));
  T81_TEST_CHECK(contains(first_result_artifact.stdout_text, "\"route\": \"A\""));
  T81_TEST_CHECK(contains(first_result_artifact.stdout_text, "\"route_token_ids\": [42]"));

  auto route_b_args = base_args;
  route_b_args.push_back("--input");
  route_b_args.push_back("hello world");
  const auto route_b_result = run_cli(t81_bin, route_b_args);
  T81_TEST_CHECK(route_b_result.exit_code == 0);
  T81_TEST_CHECK(contains(route_b_result.stdout_text, "\"route\": \"B\""));
  T81_TEST_CHECK(contains(route_b_result.stdout_text, "\"route_token_ids\": [9]"));

  auto route_c_args = base_args;
  route_c_args.push_back("--input");
  route_c_args.push_back("greet");
  const auto route_c_result = run_cli(t81_bin, route_c_args);
  T81_TEST_CHECK(route_c_result.exit_code == 0);
  T81_TEST_CHECK(contains(route_c_result.stdout_text, "\"route\": \"C\""));
  T81_TEST_CHECK(contains(route_c_result.stdout_text, "\"route_token_ids\": [7]"));
