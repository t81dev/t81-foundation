...
- add `examples/axion_policy_runner.cpp` plus documentation so the policy runner CLI now prints every `AxionEvent.verdict.reason` string (stack/heap/tensor/meta transitions plus the `AxRead/AxSet guard` verdicts described in RFC-0013/RFC-0009) and auditors can replay that trace without inspecting the VM sources
