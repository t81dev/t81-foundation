...
class T81ChatSession {
  ...
  struct OwnedMessage {
    std::string role;
    std::string content;
    OwnedMessage(std::string r, std::string c) : role(std::move(r)), content(std::move(c)) {}
  };
...
public:
  ...
  T81String say(const std::string& user_message, int max_tokens = 2048, float temperature = 0.72f,
...
public:
  ...
  explicit T81LlamaMind(const std::string& model_path,
                        ...
                        int threads = std::thread::hardware_concurrency(), int n_ctx = 131072,
                        int n_batch = 8192, int n_gpu_layers = 999, bool use_mlock = true);

  ~T81LlamaMind();

  // === Create a new persistent chat session ===
  std::shared_ptr<T81ChatSession> new_chat(
      const std::string& system_prompt = "You are a helpful, concise AI assistant.") {
    auto session = std::shared_ptr<T81ChatSession>(
        new T81ChatSession(this, system_prompt, next_session_id++), [this](T81ChatSession* s) {
  ...
  T81String think_stream(const std::string& prompt, int max_tokens = 2048,
                         float temperature = 0.72f, float top_p = 0.95f,
  ...
  // === Structured output ===
  T81String think_grammar(const std::string& prompt, const std::string& grammar,
                          int max_tokens = 2048, StreamCallback on_token = nullptr);

  // === Internal: called by T81ChatSession ===
  T81String generate_from_history(const std::vector<llama_chat_message>& full_history,
...
inline T81String T81ChatSession::say(const std::string& user_message, int max_tokens,
                                     float temperature, const std::vector<std::string>& extra_stop,
  ...
  std::vector<std::string> stops = {"<|eot_id|>", "<|im_end|>", "<|end_of_text|>", "<|assistant|>"};
  stops.insert(stops.end(), extra_stop.begin(), extra_stop.end());

  // Generate assistant response
  T81String response =
      mind->generate_from_history(full_msgs, max_tokens, temperature, stops, on_token);
...
inline T81String T81ChatSession::continue_response(int max_tokens,
                                                   T81LlamaMind::StreamCallback on_token) {
  ...
  full_msgs.insert(full_msgs.end(), history.begin(),
                   history.end() - 1);                         // exclude partial assistant
  full_msgs.push_back({"assistant", history.back().content});  // partial

  std::vector<std::string> stops = {"<|eot_id|>", "<|im_end|>", "<|end_of_text|>"};

  T81String more = mind->generate_from_history(full_msgs, max_tokens, 0.72f, stops, on_token);
...
inline T81String T81LlamaMind::generate_from_history(
    const std::vector<llama_chat_message>& full_history, int max_tokens, float temperature,
    const std::vector<std::string>& stop_strings, StreamCallback on_token) {
  std::lock_guard<std::mutex> lock(infer_mutex);
  consume_entropy(T81Entropy::acquire_batch(max_tokens));

  char buffer[128 * 1024];
  int len = llama_chat_apply_template(model, nullptr, full_history.data(), (int)full_history.size(),
  ...
  return think_stream(std::string(buffer, len), max_tokens, temperature, 0.95f, 1.1f, stop_strings,
                      on_token);
}
