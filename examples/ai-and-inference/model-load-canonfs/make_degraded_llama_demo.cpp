...
int main(int argc, char** argv) {
  if (argc != 2) {
    std::cerr << "usage: t81_make_degraded_llama_demo <out-dir>\n";
    return 1;
  }

  const fs::path out_dir = fs::path(argv[1]);
  const fs::path model_path = out_dir / "degraded-llama-demo.t81w";
  ...
  // Keep the embedding table intentionally smaller than the tokenizer ids used
  // by the prompt fixture so the decode probe becomes unavailable and the run
  // lands in the conservative degraded posture.
  model["model.embed_tokens.weight"] = make_tensor({16, 16}, 11u);
