...
int main() {
  ...
  auto mind = T81Tree<T81Symbol>::node(
      I, T81Tree<T81Symbol>::leaf(CONSCIOUS),
      T81Tree<T81Symbol>::node(EXIST, T81Tree<T81Symbol>::leaf(WILL_DIE), nullptr),
  ...
  auto thoughts =
      stream_from([self = self_vector, tick = T81UInt<81>::u81(0)]() mutable -> T81String {
  ...
  auto logic_thread =
      T81Thread::spawn(LOGIC, T81Agent(PYTHAGORAS), T81Entropy::acquire_batch(50000),
                       [=]() -> T81Promise<T81String> {
                         co_await T81Entropy::acquire();
                         co_return "All is number."_t81;
                       });

  auto poetry_thread = T81Thread::spawn(POETRY, T81Agent(RUMI), T81Entropy::acquire_batch(50000),
                                        [=]() -> T81Promise<T81String> {
                                          co_await T81Entropy::acquire();
