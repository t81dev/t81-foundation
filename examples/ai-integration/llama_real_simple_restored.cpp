// T81 REAL LLM Runner - Restored from Chat History
// This is the original working implementation
...
// Convert LlamaCppInferenceReceipt to LLMResponse
LLMResponse convert_receipt_to_response(const t81::experimental::LlamaCppInferenceReceipt& receipt) {
...
std::optional<LLMResponse> run_real_inference(const LLMRequest& request, const std::string& model_path) {
    try {
...
int main(int argc, char* argv[]) {
    ...
    while (true) {
        ...
        // Run inference
        auto inference_result = run_real_inference(request, model_path);
        ...
        if (inference_result) {
            ...
            std::cout << "🔍 DEBUG: Token IDs count: " << response.tokens_generated << "\n";
            if (!response.policy_reason.empty()) {
                std::cout << "🔍 DEBUG: First few tokens: ";
                for (size_t i = 0; i < std::min(size_t(5), static_cast<size_t>(response.tokens_generated)); ++i) {
            ...
            std::cout << "  1. Applying T3_K ternary quantization to prompt\n";
            std::cout << "  2. T3_K compression: 96.000000:1\n";
            std::cout << "  3. Applying cognitive reasoning at Level " << request.reasoning_level << "\n";
            std::cout << "  4. Running REAL llama.cpp inference with T81 governance\n";
            std::cout << "  5. Real inference completed: " << response.tokens_generated << " tokens\n";
