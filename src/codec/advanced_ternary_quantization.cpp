...
std::vector<float> AdvancedT3KQuantizer::dequantize(const std::vector<int8_t>& input) {
    ...
    for (int8_t val : input) {
        switch (val) {
            case -1: result.push_back(-1.0f); break;
            case 0: result.push_back(0.0f); break;
            case 1: result.push_back(1.0f); break;
            default: result.push_back(0.0f); break; // Handle invalid values
        }
    }
...
std::vector<int8_t> AdvancedT3KQuantizer::quantize_kmeans(const std::vector<float>& input) {
    ...
    std::sort(clusters.begin(), clusters.end(), 
              [](const KMeansCluster& a, const KMeansCluster& b) {
    ...
    for (int iter = 0; iter < config_.max_iterations; ++iter) {
        ...
        for (int i = 0; i < k; ++i) {
            if (!clusters[i].points.empty()) {
                float new_centroid = std::accumulate(clusters[i].points.begin(), 
                                                   clusters[i].points.end(), 0.0f) / 
...
std::vector<float> AdvancedT3KQuantizer::find_optimal_thresholds_entropy(const std::vector<float>& data) {
...
std::vector<float> AdvancedT3KQuantizer::calculate_percentiles(const std::vector<float>& data, 
...
std::vector<int8_t> TernaryTensorOps::add(const std::vector<int8_t>& a, const std::vector<int8_t>& b) {
    if (a.size() != b.size()) {
        return {};
    }
    ...
    for (size_t i = 0; i < a.size(); ++i) {
        ...
        else if (sum < -1) result.push_back(-1);
        else result.push_back(static_cast<int8_t>(sum));
    }
    
    return result;
}

std::vector<int8_t> TernaryTensorOps::multiply(const std::vector<int8_t>& a, const std::vector<int8_t>& b) {
    if (a.size() != b.size()) {
        return {};
    }
...
std::vector<int8_t> TernaryTensorOps::scale(const std::vector<int8_t>& input, int8_t scalar) {
    ...
    for (int8_t val : input) {
        ...
        // Clamp to ternary range
        if (product > 1) result.push_back(1);
        else if (product < -1) result.push_back(-1);
        else result.push_back(static_cast<int8_t>(product));
    }
...
void TernaryQuantizationCache::put(const std::string& key, const std::vector<int8_t>& quantized_data) {
    evict_lru_if_needed();
...
void TernaryQuantizationCache::remove(const std::string& key) {
    cache_.erase(key);
}
...
size_t TernaryQuantizationCache::size() const {
    return cache_.size();
}
...
bool is_valid_ternary(const std::vector<TernaryType>& input) {
    return std::all_of(input.begin(), input.end(),
                      [](TernaryType val) { 
                          return val == TernaryType::NEGATIVE || 
                                 val == TernaryType::ZERO || 
                                 val == TernaryType::POSITIVE; 
                      });
}
