...
void TensorMemoryPool::compact() {
  ...
  blocks_.erase(std::remove_if(blocks_.begin() + 1, blocks_.end(),
...
std::optional<std::vector<GGUTensorInfo>> EnhancedGGUFParser::parse_tensor_info(
...
std::optional<std::vector<float>> EnhancedGGUFParser::load_tensor_float(
...
std::optional<std::vector<int8_t>> EnhancedGGUFParser::load_tensor_quantized(
    const std::string& file_path, const GGUTensorInfo& tensor_info) {
  std::lock_guard<std::mutex> lock(cache_mutex_);
...
bool EnhancedGGUFParser::validate_tensor(const std::string& file_path,
...
template <typename T>
std::optional<std::vector<T>> EnhancedGGUFParser::load_tensor_data(
    const std::string& file_path, const GGUTensorInfo& tensor_info) {
  std::ifstream file(file_path, std::ios::binary);
  if (!file.is_open()) {
    return std::nullopt;
...
void EnhancedGGUFParser::evict_from_cache_if_needed() {
  ...
  while (total_cache_memory > MAX_CACHE_MEMORY || tensor_cache_.size() > MAX_CACHE_SIZE) {
    auto oldest_it = std::min_element(
        tensor_cache_.begin(), tensor_cache_.end(),
...
size_t calculate_tensor_size(const GGUTensorInfo& tensor) { return tensor.byte_size; }
...
std::string type_to_string(GGUFValueType type) {
  switch (type) {
    ...
    case GGUFValueType::UINT8:
      return "UINT8";
    case GGUFValueType::T3_K_QUANTIZED:
      return "T3_K_QUANTIZED";
    case GGUFValueType::BASE81_QUANTIZED:
      return "BASE81_QUANTIZED";
    default:
      return "UNKNOWN";
  }
}
...
float get_quantization_ratio(GGUFValueType original_type, GGUFValueType quantized_type) {
  ...
  if (original_type == GGUFValueType::FLOAT32)
    original_size = 4;
  else if (original_type == GGUFValueType::FLOAT64)
    original_size = 8;

  if (quantized_type == GGUFValueType::T3_K_QUANTIZED)
    quantized_size = 1;
  else if (quantized_type == GGUFValueType::BASE81_QUANTIZED)
    quantized_size = 1;
