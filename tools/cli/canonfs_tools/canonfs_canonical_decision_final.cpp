...
void CanonicalDecisionDemo::generate_canonical_decision() {
    ...
    rationale << "Strategy " << best_strategy << " selected based on neural network confidence " 
               << max_confidence << " with policy validation";
    decision.policy_rationale = rationale.str();
    ...
    reasoning << "Neural network analysis identified high_latency_pattern with confidence 0.87, "
               << "recommending async_operations based on performance metrics and policy constraints";
    decision.reasoning_chain = reasoning.str();
