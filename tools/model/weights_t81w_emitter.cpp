      ...
      default:
        if (static_cast<unsigned char>(c) < 0x20) {
          oss << "\\x" << std::hex << std::uppercase << std::setw(2) << std::setfill('0')
              << (static_cast<int>(c) & 0xFF) << std::dec << std::nouppercase;
        } else {
          oss << c;
        }
    }
...
std::string emit_t81w_module(const ModelFile& mf, const std::string& source_name) {
  ...
  for (size_t idx = 0; idx < mf.tensors.size(); ++idx) {
    ...
    oss << "let __t81w_tensor_" << idx << "_name: String = \"" << escape_string(tensor.name)
        << "\";\n";
    oss << "let __t81w_tensor_" << idx << "_trits: i32 = " << static_cast<int64_t>(tensor.num_trits)
        << ";\n";
