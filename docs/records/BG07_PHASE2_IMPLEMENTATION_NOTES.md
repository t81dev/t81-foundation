...
- ✅ Runtime hardening slice landed: `Frac2I` now fails closed on non-`int64` BigInt numerators (deterministic `DecodeFault`) instead of relying on uncaught overflow behavior (`vm/vm.cpp`, `tests/cpp/test_vm_literal_pool_extension.cpp`).
