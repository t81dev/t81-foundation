...
| Axion OS Kernel | Medium | Experimental RFC-00B3 path active; kernel-owned handoff, MMU fault reporting, persistent runtime state, active device arbitration, runtime-owned scheduler/IPC execution, and a deterministic kernel step loop are implemented; next milestone is richer fault delivery and runtime accounting on that loop |
