...
| Axion OS Kernel | Medium | Experimental RFC-00B3 path active; kernel-owned handoff, MMU fault reporting, persistent runtime state, active device arbitration, runtime-owned scheduler/IPC execution, deterministic kernel loop, FIFO fault delivery, process-group fault policy with manual acknowledgement gate, an audit-only supervisor layer, the first service-facing runtime contract with healthy/faulted-group semantics and stable diagnostics, and the first narrow service-facing action (supervisor fault-group acknowledgement) are implemented; next milestone is supervisor-facing recovery/report flows above that boundary (tracked in `experimental/ternaryos/docs/kernel_execution_plan.md`) |
