...
| **Axion OS** | `experimental/ternaryos/`, `spec/rfcs/RFC-00B3-axion-kernel-architecture.md` | Alpha; first kernel-owned handoff implemented | Experimental developer-lane verification only | Runtime handoff from `hal_main` to `axion_kernel_main(...)` is implemented and tested, but promotion remains non-DCP and the next kernel milestone is fault-path integration. |
