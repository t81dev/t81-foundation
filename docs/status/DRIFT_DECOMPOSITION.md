...
| **Axion OS — Runtime Integration** | `spec/rfcs/RFC-00B3-axion-kernel-architecture.md` | `hal_main` must hand off to a kernel-owned runtime and converge MMU, scheduler, IPC, and device seams into one runtime unit | Initial handoff is implemented in `userland/experimental/`: `hal_main` calls `axion_kernel_main(...)`, which validates and summarizes kernel bootstrap state; pooled-vs-unbounded epoch execution/audit parity is now CI-enforced in `axion-epoch-determinism`; full kernel runtime state and fault path are not integrated yet | Kernel entry exists and scheduler/audit parity is now evidenced, but checked MMU faults are not yet consumed by a kernel-facing reporting path and the runtime object is still bootstrap-oriented rather than persistent | Add kernel fault records on top of `mmu_translate_checked()`, then connect scheduler/IPC/device state to the first persistent kernel runtime object while retaining the new epoch parity lane as the governing proof floor | @t81dev | 2026-05-15 | **Open** |
