...
| P1 | RFC-00A6 | Axion Policy Hooks for Inference and Tooling Events | **Implemented (Signed Multi-Event Axion Ledger + Replay Verification)** — deterministic policy event contract gate with reason-code coverage now validates runtime observability trace binding (`ai_runtime_trace.json`) and emits signed multi-event Axion ledger snapshot/replay artifacts (`ai_axion_policy_ledger_snapshot.*`, `ai_axion_policy_ledger_replay_verification.*`); key material now supports `material_env` secret injection with CI secret-env wiring | Bind org/repo secrets to KMS-backed rotation policy and attach policy escalation mapping to ledger failure taxonomy | @t81dev | 2026-03-18 |
| P1 | RFC-00A5 | LLM Backend Adapter Interface (Engine-Agnostic) | **Implemented (Signed Backend-Selection Manifest Bound to Policy/Runtime Evidence)** — deterministic adapter-contract gate probes `t81_ai` (`--help`, `backend capabilities`, `backend select`, `model inspect`, `verify determinism`), emits selection trace + deterministic replay artifacts, and now signs `runtime_backend_selection_manifest.*` bound to policy contract/runtime trace/Axion ledger snapshot evidence with `material_env` support and CI secret-env wiring | Bind backend-selection key material to KMS-backed secret rotation and add escalation policy for manifest-signature failures | @t81dev | 2026-03-20 |
| P2 | RFC-00A7 | UX Integration for AI in T81 | **Implemented (Direct Backend Execution Attestations + Deterministic Evidence Constraints)** — AI CI validates runtime command surface (`model inspect`, `verify determinism`, `backend capabilities`, `backend select`, `inference run`, `quantization inspect`, `benchmark run`, `policy test`, `workflow run/replay/report`, `observability trace`) with fixture-backed GGUF binding, backend-selection trace linkage, replay-policy consistency checks, and direct backend execution attestation artifact (`ai_direct_backend_execution_attestation.*`) using governed `t81 llama-run` deterministic replay | Move direct-backend attestation policy/signing material to secret-managed keys and add escalation mapping for backend attestation failures | @t81dev | 2026-03-22 |
