...
| P3 | RFC-0026 | AI-Native Inference Opcodes | **Implemented (Phase-1 Partial Runtime Semantics + Conformance Gate)** — `tisc::Opcode` includes `ATTN`/`QMATMUL`/`EMBED`; `ATTN` and `EMBED` execute deterministic phase-1 semantics (ATTN currently uses provisional operand encoding), `QMATMUL` remains fail-closed; AI CI runs ATTN/EMBED semantic conformance plus fail-closed coverage | Land canonical 4-source opcode encoding and implement `QMATMUL` runtime semantics with shape/output conformance vectors | @t81dev | 2026-03-30 |
