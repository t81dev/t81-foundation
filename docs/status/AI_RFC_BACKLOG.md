...
| P0 | RFC-00A1 | Deterministic Evidence and Reproducibility Protocol for AI Workloads | **Implemented (Fixture-Locked Replay Vectors + Cross-Lane Hash Lock)** — canonical evidence bundle now binds to fixture GGUF replay vectors, AI CI includes VM trace-level ctest evidence (`t81_vm_trace_test`, `canonfs_axion_trace_test`) and cross-lane evidence continuity gate (`ai_cross_lane_evidence.*`) across evidence/UX/TLOADHASH artifacts | Extend evidence lock from hash continuity checks to signed multi-lane evidence manifests and promotion-window attestations | @t81dev | 2026-03-10 |
...
| P1 | RFC-0025 | Policy-Gated Tensor Loading via CanonFS | **Implemented (Governed Runtime Lane Required)** — AI CI runs VM `TLOADHASH` conformance + `t81 canonize-*` toolchain artifact and now requires governed `llama-run` evidence using sanctioned fixture `tests/fixtures/llama_cpp_repro/model.gguf` | Extend required governed lane with deterministic multi-seed replay attestations and failure taxonomy artifact | @t81dev | 2026-03-15 |
...
| P1 | RFC-00A5 | LLM Backend Adapter Interface (Engine-Agnostic) | **Implemented (Runtime Capability + Selection Trace Binding)** — deterministic adapter-contract gate and artifact in AI CI now probes `t81_ai` (`--help`, `backend capabilities`, `backend select`, `model inspect`, `verify determinism`) with JSON schema validation, emitted backend selection trace artifact (`runtime_backend_selection_trace.json`), and hashed runtime outputs | Extend selection-trace binding from deterministic single-request probes to fixture-linked multi-request selection replay attestations | @t81dev | 2026-03-20 |
| P2 | RFC-00A7 | UX Integration for AI in T81 | **Implemented (Fixture-Bound UX + Policy-Linked Replay Assertions)** — AI CI validates runtime command surface (`model inspect`, `verify determinism`, `backend capabilities`, `inference run`, `quantization inspect`, `benchmark run`, `policy test`, `workflow run/replay/report`, `observability trace`) with fixture-backed GGUF binding and replay-policy linkage checks (`policy_reason_code`/`policy_decision`) | Replace deterministic stub outputs with live backend execution semantics while preserving evidence determinism constraints | @t81dev | 2026-03-22 |
