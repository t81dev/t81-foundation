...
| P1 | RFC-00A5 | LLM Backend Adapter Interface (Engine-Agnostic) | **Implemented (Baseline + Runtime Binding Probe)** — deterministic adapter-contract gate and artifact in AI CI now includes runtime binding probe against `t81_ai` (`--help`, `model inspect`, `verify determinism`) with hashed runtime outputs | Expand runtime probe from command-surface binding to explicit backend capability introspection schema emitted by binary | @t81dev | 2026-03-20 |
