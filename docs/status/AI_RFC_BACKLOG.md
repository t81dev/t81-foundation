...
| P3 | RFC-0026 | AI-Native Inference Opcodes | **Partial (Phase-1 Runtime Semantics + Conformance Gate + Ready Readiness Tracker)** — `tisc::Opcode` includes `ATTN`/`QMATMUL`/`EMBED`; all three execute deterministic phase-1 runtime semantics with canonical packed-operand phase-1 encoding for `ATTN`/`QMATMUL`. The VM preserves exact-input deterministic fixed-math paths for `softmax`, attention, `rmsnorm`, and `rope`, while broader host-float tensor lanes remain bounded runtime behavior rather than universal strict-core deterministic execution. AI CI runs semantic conformance for the phase-1 subset, emits RFC-0026 readiness tracker artifacts (`ai_rfc0026_readiness.*`), enforces runtime capability alignment (`ai_runtime_capability_alignment.json`), binds live WLOAD allow-path runtime evidence through the policy-event contract/runtime trace path, and now reports governed strict-deterministic `gguf` + `t3k` readiness via the `t81_reference_vm` reference backend. | Replace remaining host-float tensor storage and general float-input execution paths before broad strict-core promotion claims | @t81dev | 2026-03-30 |
