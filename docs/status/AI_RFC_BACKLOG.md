...
| P0 | RFC-00A1 | Deterministic Evidence and Reproducibility Protocol for AI Workloads | **Implemented (Signed Multi-Lane Evidence Manifest + Promotion Window Attestation)** — canonical evidence bundle binds fixture GGUF replay vectors; AI CI includes VM trace-level ctest evidence (`t81_vm_trace_test`, `canonfs_axion_trace_test`), cross-lane continuity gate (`ai_cross_lane_evidence.*`), and signed multi-lane manifest artifact (`ai_evidence_manifest.*`) with promotion-window attestation metadata | Upgrade deterministic manifest attestation from hash-only signing to key-backed signature verification and rotation policy | @t81dev | 2026-03-10 |
