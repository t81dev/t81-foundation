...
| P3 | RFC-0026 | AI-Native Inference Opcodes | **Implemented (Phase-1 Runtime Semantics + Conformance Gate + Readiness Tracker + Runtime Lane Alignment Guard)** — `tisc::Opcode` includes `ATTN`/`QMATMUL`/`EMBED`; all three execute deterministic phase-1 runtime semantics with canonical packed-operand phase-1 encoding for `ATTN`/`QMATMUL`; AI CI runs semantic conformance for ATTN/EMBED/QMATMUL, emits RFC-0026 readiness tracker artifact (`ai_rfc0026_readiness.*`) binding opcode evidence with benchmark and inference lane capability states (including tracked `t3k` unsupported lanes), and enforces benchmark/inference capability alignment for governed format/mode pairs (`ai_runtime_capability_alignment.json`) | Land WLOAD policy-gated runtime evidence and ternary-capable benchmark/inference lane support to clear RFC-0026 readiness blockers | @t81dev | 2026-03-30 |
