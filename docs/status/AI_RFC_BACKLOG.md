...
| P3 | RFC-0026 | AI-Native Inference Opcodes | **Implemented (Phase-1 Runtime Semantics + Conformance Gate)** — `tisc::Opcode` includes `ATTN`/`QMATMUL`/`EMBED`; all three now execute deterministic phase-1 runtime semantics (`ATTN`/`QMATMUL` currently use provisional operand/runtime paths); AI CI runs semantic conformance for ATTN/EMBED/QMATMUL | Land canonical 4-source opcode encoding and harden QMATMUL from provisional deterministic matmul path to finalized quantized runtime semantics | @t81dev | 2026-03-30 |
| P3 | RFC-00A8 | AI-Native VM Opcode Exploration | **Implemented (Runtime-Bound Dispatch + Execution Evidence Report)** — RFC-00A8 exploration constrained by RFC-0026 subset contract and CI-published opcode runtime report (`ai_opcode_runtime_report.*`) plus `/experiments/ai/opcodes/IMPLEMENTATION_REPORT.md`, bound to phase-1 conformance ctest evidence (`ai_phase1_opcode_ctest.log`) and per-opcode deterministic output hashes | Expand report with richer fixture/vector provenance metadata and baseline-vs-current hash diffing for promotion gates | @t81dev | 2026-04-02 |
