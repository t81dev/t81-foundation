...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| P0 | RFC-0000 | T81 Base-81 Ternary Computing Stack | **Partial (7/7 core items implemented 2026-03-08)** — (1) `EthicsViolation`/`CapabilityDenied` fault types added to `Trap` enum; AXHALT `insn.a` field distinguishes ethics vs capability halt. (2) `CanonBlock` 729-tryte type added to `canon_types.hpp` with `hash()`/`to_bytes()`/`from_bytes()`. (3) Hanoi 81-slot scheduler: `spawn()` capped at 81 active PIDs; `Error::SchedulerFull` added. (4) Ethics-first boot: `Kernel::boot()` interface added; `InMemoryKernel::boot()` evaluates Θ₁–Θ₉ via `check_ethics()` before first `spawn()`. (5) Axion command surface: `t81 axion status\|optimize\|simulate\|snapshot\|rollback` CLI subcommands wired to `run_axion()`. (6) T6561 Tier 6 cognition: `TierId::Tier6` enum value, `MeshReflector`/`MonadState` types in `tier6/distributed_monad.hpp`, Θ₇ entropy containment gate, `promotion.cpp` Tier5→Tier6 path. (7) CanonHash-81 reference vectors: `canonhash81_reference_vectors_test.cpp` with 8 vectors (empty, single-byte, known-string, all-zeros block, all-ones block, round-trip, wrong-size rejection, Base-81 determinism). | CanonHash-81 formal test-vector publication (run + record expected digests); CanonFS parity 3+2 Reed-Solomon; formal Θ-overlay soundness proofs; post-quantum CanonHash parameterisation | @t81dev | 2026-04-30 |
...
| P3 | RFC-0026 | AI-Native Inference Opcodes | **Partial (AI-M1–M6 Complete; host-float path replacement remaining)** — All six phase-1 opcodes implemented and spec-documented. AI-M4: WLOAD CanonFS audit gate (`action=WeightLoad`) gated on `T81_CANONFS_ROOT`. AI-M5: multi-axis GATHER (axis=0/1) + SCATTER aliasing detection via `ThreadContext::scatter_used`. AI-M6 (2026-03-07): T81Lang `@tier(2) @attention fn` and `@tier(2) @qmatmul fn` annotations lower call sites to ATTN/QMATMUL opcodes; `Tensor.attention(q,k,v)` and `Tensor.qmatmul(act,wt,scale)` immediate builtins also supported; SA enforces Tier 2+ requirement for both annotations; tested by `lang_ai_m6_annotation_lowering_test` (5 assertions: builtin lowering ×2, annotated call site lowering ×2, Tier-1 rejection). Spec docs: `spec/tisc-spec.md §5.15`, `opcode-registry.md §2.18`, `opcode-unified-reference.md §5.18` complete. IR pipeline: `ir::Opcode::{ATTN,QMATMUL,WLOAD,GATHER,SCATTER}` added; `binary_emitter.cpp` maps all five to tisc opcodes; `ai_native_opcodes.hpp` `is_phase1_ai_opcode()` extended to full set. Deterministic output hashes: `WLOAD=77ed877f762ee0bd`, `GATHER=8715026970757876`, `SCATTER=36872c2021d93029`. | Replace remaining host-float tensor storage paths for broad strict-core determinism promotion | @t81dev | 2026-03-30 |
...
- Partial RFCs: `RFC-0000` (7 core items complete; parity codec, formal proofs, post-quantum hash remain open), `RFC-0026` (AI-M1–M6 complete; host-float path replacement remains open)
...
- Baseline CI-gated RFCs: `RFC-00A0`, `RFC-00A1`, `RFC-00A2`, `RFC-00A3`, `RFC-00A4`, `RFC-00A5`, `RFC-00A6`, `RFC-00A7`, `RFC-0025`, `RFC-0026`, `RFC-00A8`
- Partial RFCs: `RFC-0026` (AI-M1–M5 + spec docs complete; AI-M6 T81Lang lowering and host-float path replacement remain open)
- Current gating principle: no AI feature promotion claims without deterministic evidence + provenance + policy enforcement
