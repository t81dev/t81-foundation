...
| P1 | RFC-0025 | Policy-Gated Tensor Loading via CanonFS | **Implemented (Toolchain Evidence Active)** — AI CI runs VM `TLOADHASH` conformance + `t81 canonize-*` toolchain evidence artifact with policy allowlist snippet | Add demo/runtime execution evidence that consumes generated hash in governed inference flow | @t81dev | 2026-03-15 |
