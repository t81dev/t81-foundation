...
| P3 | RFC-0026 | AI-Native Inference Opcodes | **Implemented (Phase-1 Runtime Semantics + Conformance Gate + Readiness Tracker + Runtime Lane Alignment Guard)** — `tisc::Opcode` includes `ATTN`/`QMATMUL`/`EMBED`; all three execute deterministic phase-1 runtime semantics with canonical packed-operand phase-1 encoding for `ATTN`/`QMATMUL`; AI CI runs semantic conformance for ATTN/EMBED/QMATMUL, emits RFC-0026 readiness tracker artifact (`ai_rfc0026_readiness.*`) binding opcode evidence with benchmark and inference lane capability states, enforces benchmark/inference capability alignment for governed format/mode pairs (`ai_runtime_capability_alignment.json`) with governed expectations contract (`ai_runtime_capability_alignment_expectations.json`), tracks WLOAD policy-evidence readiness as a governed dependency artifact (`ai_wload_policy_evidence.json`) with expectation contract (`ai_wload_policy_evidence_expectations.json`) covering readiness state and WLOAD reason-code prefix/min-count policy, now publishes explicit allow-path + deny-path WLOAD policy evidence (`AI_POLICY_ALLOW_WLOAD_POLICY_GATE`, `AI_POLICY_DENY_WLOAD_UNSUPPORTED`), requires readiness consistency with alignment/WLOAD/opcode-baseline gate status, and enforces governed readiness expectation contract (`ai_rfc0026_readiness_expectations.json`) across blocker taxonomy plus WLOAD evidence/status signals during phased rollout | Maintain readiness evidence as opcode baselines and capability contracts evolve | @t81dev | 2026-03-30 |
