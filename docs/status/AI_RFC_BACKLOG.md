...
| P3 | RFC-0026 | AI-Native Inference Opcodes | **Implemented (Phase-1 Runtime Stubs + Conformance Gate)** — `tisc::Opcode` now includes `ATTN`/`QMATMUL`/`EMBED`; VM dispatch stubs fail-closed with deterministic deny reason; AI CI runs `t81_vm_ai_phase1_opcode_stub_conformance_test` | Replace fail-closed stubs with full opcode semantics + tensor/shape/output conformance vectors | @t81dev | 2026-03-30 |
