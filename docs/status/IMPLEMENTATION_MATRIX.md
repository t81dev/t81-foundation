...
| **SIMD Tritwise Acceleration** | RFC-0041 | **Accepted** | **Beta** | **Beta** | Medium | Medium | 2026-03-18 | @t81dev | 2026-04-15 | **✅ ACCEPTED** - Core AVX2/NEON packed-trit kernels, threshold dispatch, SWAR tail fallback, backend-equivalence tests, benchmark coverage, stable `include/t81/simd/simd.hpp`, migration guidance, ARM64 evidence, and tuned ARM64 dispatch policy are in place. Remaining work is refreshed x86_64 evidence plus compatibility/deprecation closure for the next promotion step. |
| **TernaryOS User Environment** | RFC-00B9 | **Accepted** | **Beta** | **Beta** | Low | Medium | 2026-03-18 | @t81dev | 2026-06-15 | **✅ ACCEPTED (OPT-IN BUILD)** - RFC-00B9 acceptance coverage is implemented and passing in the opt-in TernaryOS build (`-DT81_ENABLE_TERNARYOS=ON`): `t81-init`, session manager, `t81sh`, service activation, shell-session durability, and the AC-1..15 acceptance target all pass. This is not default-on or production-ready in the main build yet; remaining work is promotion hardening and integration beyond the experimental TernaryOS configuration. |
...
| **Ternary-Native Inference** | RFC-0034 + RFC-0037 | **Accepted** | **Beta** | **Beta** | Medium | Medium | 2026-03-18 | @t81dev | 2026-06-15 | **✅ ACCEPTED** - RFC-0034 implementation work is closed in-repo: `TWMATMUL`, `TQUANT`, `TATTN`, `TWEMBED`, `TERNACCUM`, `TACT`; `std.tnn.*` lowering; `@ternary_inference` lowering for compatible `std.tensor.*` AI calls; `activation-ceiling`, `allowed-ternary-model-hashes`, and `ternary-weight-domain-check` enforcement for model-backed `WLOAD`; authored conformance programs; CanonFS-backed `.t81w` CLI execution evidence; native benchmark evidence, including same-build substrate comparison against llama.cpp; and an initial native VM fast-path set for balanced-trit weights (`TExp`, `TQUANT`, `TACT`, `TERNACCUM`, `TSiLU`, `TSoftmax`, `TRMSNorm`, `TRoPE`, `TWEMBED`, `TWMATMUL`, `TATTN`) with matched VM native-vs-binary wins recorded at roughly `62x-67x` for the unary set, `6259.43x` for `TQUANT`, `6834.34x` for `TACT`, `80.57x` for `TERNACCUM`, `11.99x` for `TRMSNorm`, `4.07x` for `TRoPE`, `769.63x` for `TWEMBED`, a scale-dependent `TWMATMUL` crossover from `0.84x` at `64` to `11.84x` at `256` and `570.42x` at `4096`, and `TATTN` at `110970.48x` on the current `256`-trit VM comparison. Remaining work is integration/stable-promotion hardening: cross-platform evidence refresh plus broader optimization of native result representation after decode. |
...
| **Governed FFI** | RFC-00B8 + RFC-0036 | **Accepted** | **Beta** | **Beta** | Medium | Medium | 2026-03-18 | @t81dev | 2026-05-15 | **✅ ACCEPTED** - RFC-0036 language work and the RFC-00B8 VM bridge are wired end to end: `FFIDispatcher`, `FFILibraryRegistry`, `FFICall`/`FFIRegister`/`FFIPolicySet`, symbol-preserving `FFI_CALL` emission, and VM bridge tests covering success, failure, audit-trail, and quarantine paths. The current bridge covers integer scalar calls, string transport, float transport, length-aware bytes transport including embedded-NUL payloads, mixed-type ordered argument lists, initial structured argument/return paths via `StringVectorHandle`, and real system-library evidence (`getpid`, `llabs`, `strlen`/`lstrlenA`, `fabs`). Remaining work is stable-promotion hardening: broader structured schemas, more ecosystem bindings, and deciding whether quarantined FFI needs a true sandbox boundary instead of governed dynamic loading. |
