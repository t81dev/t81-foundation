...
| **SIMD Tritwise Acceleration** | RFC-0041 | **Proposed** | **Beta** | **Beta** | Medium | Medium | 2026-03-18 | @t81dev | 2026-04-15 | AVX2 and NEON packed-trit kernels, threshold dispatch, SWAR tail fallback, backend-equivalence tests, and benchmark coverage already exist. Stable public surface is now `include/t81/simd/simd.hpp`; migration guidance exists; ARM64 evidence is recorded in `RFC_0041_SIMD_EVIDENCE_2026-03-18.md`. Remaining work is x86_64 evidence refresh, possible ARM64 threshold tuning, and compatibility/deprecation closure. |
...
| **Ternary-Native Inference** | RFC-0034 + RFC-0037 | **Proposed** | **Beta** | **Beta** | Medium | Medium | 2026-03-18 | @t81dev | 2026-06-15 | RFC-0034 implementation work is closed in-repo: `TWMATMUL`, `TQUANT`, `TATTN`, `TWEMBED`, `TERNACCUM`, `TACT`; `std.tnn.*` lowering; `@ternary_inference` lowering for compatible `std.tensor.*` AI calls; `activation-ceiling`, `allowed-ternary-model-hashes`, and `ternary-weight-domain-check` enforcement for model-backed `WLOAD`; authored conformance programs; CanonFS-backed `.t81w` CLI execution evidence; native benchmark evidence, including same-build substrate comparison against llama.cpp; and an initial native VM fast-path set for balanced-trit weights (`TExp`, `TQUANT`, `TACT`, `TERNACCUM`, `TSiLU`, `TSoftmax`, `TRMSNorm`, `TRoPE`, `TWEMBED`, `TWMATMUL`, `TATTN`) with matched VM native-vs-binary wins recorded at roughly `62x-67x` for the unary set, `6259.43x` for `TQUANT`, `6834.34x` for `TACT`, `80.57x` for `TERNACCUM`, `11.99x` for `TRMSNorm`, `4.07x` for `TRoPE`, `769.63x` for `TWEMBED`, a scale-dependent `TWMATMUL` crossover from `0.84x` at `64` to `11.84x` at `256` and `570.42x` at `4096`, and `TATTN` at `110970.48x` on the current `256`-trit VM comparison. Remaining work is cross-platform promotion evidence refresh plus broader optimization of native result representation after decode. |
