...
| BG-07 | Scope reduction / determinism tightening | `T81BigInt` | VM aliases BigInt to 64-bit; >64-bit literals silently truncated — spec claims arbitrary-precision | Native arbitrary-precision opcode path OR explicit governance decision to narrow spec claim. Phase-2 planning notes captured in `docs/status/BG07_PHASE2_IMPLEMENTATION_NOTES.md` (2026-03-05); scaffolding slice landed for ISA/program BigInt literal transport (`LiteralKind::BigIntHandle`, `Program::bigint_pool`, emitter/IO wiring), VM `LOADI` now materializes BigInt literals as canonical `FractionHandle` (`BigInt/1`), frontend oversized decimal/base81 integer literals now lower to `BigIntHandle`, binary round-trip conformance coverage explicitly validates BigInt literal pool transport determinism, `Frac2I` overflow now fails closed on non-`int64` BigInt numerators, and JIT `LoadImm` explicitly deopts `BigIntHandle` to preserve interpreter precision semantics (2026-03-05). | @t81dev | 2026-05-15 | Open |
