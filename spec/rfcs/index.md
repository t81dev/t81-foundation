...
| RFC-0040 | SWAR Formalization | accepted | In-repo implementation closure is complete: stable SWAR API, explicit TISC opcodes (`0xD5`-`0xD7`), Setun bridge mnemonics, VM/JIT/CanonFS coverage, Axion trace/policy coverage, and performance evidence are all in place; remaining work is refreshed cross-architecture bit-exact evidence for the next promotion step |
| RFC-0041 | SIMD Formalization | accepted | In-repo implementation closure is complete at the library/API level: core AVX2/NEON packed-trit kernels, stable `include/t81/simd/simd.hpp` facade, migration guide, ARM64 evidence record, and tuned ARM64 dispatch policy are all in place; remaining work is refreshed x86_64 evidence plus compatibility/deprecation closure for the next promotion step |
...
| RFC-0055 | Native Ternary Hardware Target and Interop Contract | draft | Separates native ternary hardware targets from accelerator-only targets; defines TISC semantic authority, hardware interop obligations, and promotion rules for future external ternary platforms |
| RFC-0056 | Google Axion-T81 Integration Contract | draft | Defines integration contract for deploying T81 on Google Axion processors, establishing governed path for leveraging ARM Neoverse V2 architecture while preserving ternary semantic guarantees |
...
| RFC-00B8 | Governed Foreign Function Interface | accepted | RFC-0036 language surface and the RFC-00B8 VM bridge are implemented end to end: encoded function-name symbols survive emission into real `FFICall`/`FFIRegister` dispatch, the bridge covers integer/string/double/bytes/mixed/initial-structured shapes, and VM evidence now includes success, failure, audit-trail, quarantine, and real system-library calls; stable promotion still needs broader schemas/bindings and an explicit sandbox decision |
| RFC-00B9 | TernaryOS User Environment Standard | accepted | `t81-init`, session model, `t81sh`, service registry, TTY contract, and Axion gates (`BootService`/`SessionCreate`/`ServiceSpawn`/`ShellExec`) are implemented with AC-1..15 passing in the opt-in TernaryOS build (`-DT81_ENABLE_TERNARYOS=ON`); default-on/runtime promotion remains beta work |
...
| RFC-00BB | Native Model Architecture Compatibility | accepted | Compatibility-state/profile gating is active in code: `bitnet-b1.58-v1`, `native-dense-v1` fallback, `gemma-dense-v1`, `llama-dense-v1`, `mistral-dense-v1`, `phi3-dense-v1`, and `qwen2-dense-v1` are the current experimental-native profiles, with deterministic rejection rules, real TinyLlama conversion evidence, and a real-family `GGUF -> .t81w -> VM TWEMBED` execution proof for `llama-dense-v1`; further family-by-family runtime evidence remains post-acceptance hardening |
