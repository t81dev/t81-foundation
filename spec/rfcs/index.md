...
| RFC-00BA | llama.cpp GGUF Ingestion Bridge | proposed | The narrow llama.cpp-backed GGUF bridge now exists in code: build-gated metadata enumeration, float export, and bridge-backed `weights import --format gguf` conversion are implemented while runtime execution remains independent from llama.cpp; remaining work is provenance/revision hardening before `accepted` |
| RFC-00BB | Native Model Architecture Compatibility | proposed | Compatibility-state/profile gating is active in code: `bitnet-b1.58-v1`, `native-dense-v1` fallback, `gemma-dense-v1`, `llama-dense-v1`, `mistral-dense-v1`, `phi3-dense-v1`, and `qwen2-dense-v1` are the current experimental-native profiles, with deterministic rejection rules and first real TinyLlama conversion evidence in place; remaining work is family-specific end-to-end execution evidence before `accepted` |
