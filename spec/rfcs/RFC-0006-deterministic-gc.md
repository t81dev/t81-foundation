...
| [A-0006-02] | §2.2 Deterministic heap sweep: `compact_heap()` resets heap_frames and heap_ptr deterministically | met — `core/vm/gc_helpers.cpp` called from `run_gc_cycle_()` in `vm/vm.cpp` |
| [A-0006-03] | §2.3 `GcSafepoint` opcode: assigned in ISA, dispatched in VM, triggers `run_gc_cycle_()` | met — `Opcode::GcSafepoint` in `include/t81/isa/opcodes.hpp`; handler in `vm/vm.cpp`; proved by `vm_gc_safepoint_test` [RFC-0006-§2.3-a,b] |
| [A-0006-04] | §2.3 Byte-threshold trigger: `kGcThreshold = 531441` (`3^12` bytes) alongside instruction-count trigger | met — `heap_bytes_since_gc_` accounting at Alloc site; `kGcThreshold` constant; dual condition in `vm/vm.cpp`; proved by `vm_gc_safepoint_test` [RFC-0006-§2.3-c] |
...
`vm/vm.cpp` calls `run_gc_cycle_("safepoint")` on opcode hit.
