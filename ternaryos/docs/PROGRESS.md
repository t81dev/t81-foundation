...
- RFC-00C5 (concurrent device wait) — **implemented (Phase 16)**; RFC-00C6 (per-thread TTBR0 address-space isolation) — **implemented (Phase 17)**: two threads (tid=6/did=30 exact, tid=7/did=0 wildcard) park on `WaitForDevice` simultaneously using two independent EL0 code+stack page pairs; single timer IRQ wakes both via `fs_sched_timer_device_wake(30)`; `fs_sched_exit_thread` chains the two ExitThread returns without EL1 re-entry; MMU extended with `proc_code_page2` + `proc_stack_page2`; `el0_tva_valid` covers all six pages; Phase 16 CI gate: `[axion] el0: concurrent wake OK (device_id=30, tid=6+7)`. RFC-00C6 (per-thread address-space isolation): each thread gets a private L3 table (cloned from shared, other thread's proc pages set to `AP=0b00`/`UXN`); `el0_mmu_install_thread_l3()` swaps `s_l2[block_idx]`+TLBI before each ERET; `el0_mmu_install_shared_l3()` restores on EL1 return; `kNoThreadL3=~0u` sentinel preserves compat for all prior phases; Phase 17 CI gate: `[axion] el0: per-thread pt OK (tid=6+7, isolated)`.
