...
static void test_scripted_shell_session() {
  ...
  check(state->available_commands.size() == 33, "thirty-three builtins are exposed");
...
static void test_typed_shell_commands() {
  ...
  const auto stored_ref = suffix_after(session->state().command_records[6].result, "canon durable ok ");
  ...
  const auto checkpoint_ref =
      suffix_after(session->state().command_records[9].result, "session checkpoint ok ");
  ...
  const auto export_ref =
      suffix_after(session->state().command_records[10].result, "session export ok ");
  ...
  const auto copied_ref = suffix_after(session->state().command_records[17].result, "canon durable ref ok ");
  ...
  const auto cp_ref = suffix_after(session->state().command_records[18].result, "store cp ok ");
  ...
  check(state_before_clear.command_records.size() == 31, "interactive session records thirty-one commands");
  ...
  check(state_before_clear.command_records[3].result.starts_with("session profile "),
        "session status reports profile and shell metadata");
  check(state_before_clear.command_records[4].result == "show profile\n" + state_before_clear.profile_summary,
        "show profile reports object-native profile view");
  check(state_before_clear.command_records[5].result.starts_with("show session\nprofile "),
        "show session reports object-native session view");
  check(state_before_clear.command_records[6].command == "store put \"typed shell payload\"",
        "typed parser preserves quoted store put command");
  check(state_before_clear.command_records[6].result.starts_with("canon durable ok "),
        "store put reports durable success plus CanonRef");
  check(state_before_clear.command_records[7].result.find("anchor present") != std::string::npos,
        "session show durable reports durable anchor state");
  check(state_before_clear.command_records[7].result.find(stored_ref) != std::string::npos,
        "session show durable exposes the tracked CanonRef");
  check(state_before_clear.command_records[8].result.starts_with("session refs 1"),
  ...
  check(state_before_clear.command_records[9].result.starts_with("session checkpoint ok "),
        "session checkpoint reports durable success");
  check(state_before_clear.command_records[9].result.find(checkpoint_ref) != std::string::npos,
        "session checkpoint returns a CanonRef");
  check(state_before_clear.command_records[10].result.starts_with("session export ok "),
        "session export reports durable success");
  check(state_before_clear.command_records[10].result.find(export_ref) != std::string::npos,
  ...
  check(state_before_clear.command_records[11].result.starts_with("session diff mismatch"),
        "session diff reports transcript mismatch before clear");
  check(state_before_clear.command_records[11].result.find("current>") != std::string::npos,
        "session diff reports the current transcript side");
  check(state_before_clear.command_records[11].result.find("durable>") != std::string::npos,
        "session diff reports the durable transcript side");
  check(state_before_clear.command_records[12].result.starts_with("history object " + checkpoint_ref),
        "history show object exposes the checkpoint CanonRef");
  check(state_before_clear.command_records[12].result.find("SESSION TRANSCRIPT") != std::string::npos,
        "history show object exposes the persisted transcript");
  check(state_before_clear.command_records[13].result.starts_with("show ref " + checkpoint_ref),
  ...
  check(state_before_clear.command_records[14].result.starts_with("store refs 2"),
        "store ls reports both tracked refs");
  check(state_before_clear.command_records[14].result.find(stored_ref) != std::string::npos,
        "store ls exposes the stored payload CanonRef");
  check(state_before_clear.command_records[14].result.find(checkpoint_ref) != std::string::npos,
        "store ls exposes the checkpoint CanonRef");
  check(state_before_clear.command_records[15].result == "store get typed shell payload",
        "store get decodes stored payload");
  check(state_before_clear.command_records[16].result.starts_with("show ref " + stored_ref),
        "show ref exposes the requested CanonRef");
  check(state_before_clear.command_records[16].result.find("typed shell payload") != std::string::npos,
        "show ref exposes the durable payload");
  check(state_before_clear.command_records[17].result.starts_with("canon durable ref ok "),
        "store put ref reports durable success");
  check(state_before_clear.command_records[17].result.find(copied_ref) != std::string::npos,
  ...
  check(state_before_clear.command_records[18].result.starts_with("store cp ok "),
        "store cp reports durable success");
  check(state_before_clear.command_records[18].result.find(cp_ref) != std::string::npos,
        "store cp returns a CanonRef");
  check(cp_ref == stored_ref,
        "store cp preserves canonical identity for identical payload");
  check(state_before_clear.command_records[19].result == "store get typed shell payload",
        "store get on copied ref decodes stored payload");
  check(state_before_clear.command_records[20].result.starts_with("history session 20"),
        "history show session reports the current session window");
  check(state_before_clear.command_records[20].result.find("store cp " + stored_ref) != std::string::npos,
        "history show session includes object-copy command");
  check(state_before_clear.command_records[20].result.find("store put ref " + stored_ref) != std::string::npos,
        "history show session includes object-composition command");
  check(state_before_clear.command_records[21].result.starts_with("history durable " + copied_ref),
        "history show durable exposes the durable anchor ref");
  check(state_before_clear.command_records[21].result.find("typed shell payload") != std::string::npos,
        "history show durable exposes the durable payload");
  check(state_before_clear.command_records[22].result == "store rm ok " + stored_ref,
        "store rm reports durable removal");
  check(state_before_clear.command_records[23].result == "store get missing",
        "removed ref is no longer readable");
  check(state_before_clear.command_records[24].result == "show ref missing",
        "show ref reports missing after removal");
  check(state_before_clear.command_records[25].result == "reboot history missing",
        "history reports missing durable anchor after removal");
  check(state_before_clear.command_records[26].result == "history durable missing",
        "history show durable reports missing anchor after removal");
  check(state_before_clear.command_records[27].result == "history use ok " + checkpoint_ref,
        "history use rebinds the durable anchor to the checkpoint");
  check(state_before_clear.command_records[28].result.starts_with("history durable " + checkpoint_ref),
        "history show durable exposes rebound checkpoint anchor");
  check(state_before_clear.command_records[28].result.find("SESSION TRANSCRIPT") != std::string::npos,
        "history show durable exposes rebound checkpoint transcript");
  check(state_before_clear.command_records[29].result == "parse error: unmatched quote",
        "unmatched quote is surfaced as a parse error");
  check(state_before_clear.command_records[30].result == "unknown command", "unknown command is surfaced");
  check(state_before_clear.recovered_entries == 1, "interactive history refresh tracks current recovered_entries");
  check(state_before_clear.session_command_count == 31,
