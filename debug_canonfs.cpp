...
int main() {
    ...
    options.canonfs_root = "/tmp/debug_canonfs";
    ...
    std::filesystem::create_directories("/tmp/debug_canonfs");
    
    auto outcome1 = import_path("/tmp/nonexistent_file.txt", options);
    ...
    std::filesystem::create_directories("/tmp/debug_canonfs/empty");
    
    auto outcome2 = import_path("/tmp/debug_canonfs/empty", options);
    ...
    std::cout << "\n=== Test 3: Directory with files ===" << std::endl;
    std::filesystem::create_directories("/tmp/debug_canonfs/with_files");
    std::ofstream("/tmp/debug_canonfs/with_files/test.txt") << "test content";
    
    auto outcome3 = import_path("/tmp/debug_canonfs/with_files", options);
