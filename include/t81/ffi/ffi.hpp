...
// Shared ABI-only bridge results for simple structured returns from foreign calls.
...
public:
  ...
  t81::expected<void, std::string> register_library(const std::string& library_name,
                                                    const std::string& version_hash,
                                                    const std::vector<FFIFunction>& functions);

  // Lookup function by name
  t81::expected<FFIFunction*, std::string> lookup_function(const std::string& function_name);
...
public:
  ...
  t81::expected<FFICallResult, std::string> call(const FFICallContext& context);
...
private:
  ...
  t81::expected<void, std::string> check_policy_(const FFICallContext& context,
                                                 const FFIFunction& function);

  // Execute the actual FFI call
  FFICallResult execute_call_(const FFICallContext& context, const FFIFunction& function);

  // Generate audit events
  void generate_audit_events_(const FFICallContext& context, const FFIFunction& function,
                              const FFICallResult& result);

  // Helper functions
  std::string generate_cache_key_(const FFICallContext& context, const FFIFunction& function);
  std::optional<FFICallResult> lookup_cache_(const std::string& key);
  void cache_result_(const std::string& key, const FFICallResult& result);
  std::string compute_provenance_hash_(const std::string& data);
  FFICallResult call_foreign_function_(void* func_ptr, const FFICallContext& context,
                                       const FFIFunction& function);
};
