...
struct T81Error {
    ...
    [[nodiscard]] T81String explain() const {
        return T81String("[ERROR ") + T81String(code.to_string())
             + T81String(" at ") + occurred_at.narrate()
             + T81String("] ") + message
             + T81String(" (source: ") + T81String(source.to_string()) + T81String(")");
    }
};
...
public:
    ...
    //===================================================================
    constexpr T81Result(const T& value) noexcept
        : payload_(std::in_place_index<0>, value) {}

    constexpr T81Result(T&& value) noexcept
        : payload_(std::in_place_index<0>, std::move(value)) {}

    constexpr T81Result(const T81Error& err) noexcept
        : payload_(std::in_place_index<1>, err) {}

    constexpr T81Result(T81Error&& err) noexcept
        : payload_(std::in_place_index<1>, std::move(err)) {}
    ...
    static constexpr T81Result failure(T81Symbol code,
                                       T81String msg,
    ...
    [[nodiscard]] T81Reflection<T81Result<T>> reflect() const {
        auto kind = is_ok()
            ? T81Symbol::intern("SUCCESS")
            : T81Symbol::intern("FAILURE");
        return T81Reflection<T81Result<T>>(*this,
                                           T81Symbol::intern("RESULT"),
                                           kind);
    }
...
// ======================================================================
namespace wisdom {
    inline constexpr auto SAFE_DIV =
        [](T81Int<81> a, T81Int<81> b) -> T81Result<T81Int<81>> {
            if (b.is_zero()) {
