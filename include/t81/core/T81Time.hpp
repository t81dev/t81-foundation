...
public:
    ...
    //===================================================================
    static T81Time now(T81Entropy&& fuel, T81Symbol event = T81Symbol::intern("TICK")) {
    ...
    static T81Time genesis() {
        return T81Time(T81Int<81>(0), acquire_entropy(), T81Symbol::intern("GENESIS"));
    }
    ...
    void observed_by(T81Reflection<T81Time>& observer) {
        observer_ = &observer;
        observer.observe(std::make_optional(witness_));
    }
    ...
    [[nodiscard]] T81String narrate() const {
        return T81String("At tick ") + T81String(tick_.to_string()) +
               T81String(" the event '") + T81String(event_id_.to_string()) +
