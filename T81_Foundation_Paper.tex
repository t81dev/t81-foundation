...
\begin{verbatim}
let res: Result[Option[i81], string] = Ok(Some(42)); // i81 is an 81-trit signed integer
let out = match res {
...
The results show that the HanoiVM's interpreted execution imposes a significant performance overhead. It is important to note that the current Llama bridge is a proof-of-concept designed to validate functional correctness, not to achieve state-of-the-art performance. The underlying implementation uses naive matrix multiplication and does not yet leverage modern CPU features like SIMD/AVX instructions for acceleration. As such, the performance figures in Table \ref{tab:perf_results} should be interpreted as a conservative baseline. While not yet competitive for high-performance inference, these results demonstrate that the T81 stack is capable of executing complex, real-world AI workloads correctly. This establishes a baseline from which future optimization work, such as the development of a JIT compiler for the HanoiVM, can proceed. Our central claim is not that our system is the fastest, but that it is verifiably deterministic.
