...
bool HasBenchmarkArg(int argc, char** argv, std::string_view flag_prefix,
                     std::string_view exact_flag) {
...
std::vector<std::string> BuildEffectiveBenchmarkArgs(int argc, char** argv) {
  ...
  const bool has_filter = HasBenchmarkArg(argc, argv, "--benchmark_filter=", "--benchmark_filter");
...
static bool IsNativeOnlyFamily(const BenchmarkResult& r) {
  return (r.has_t81_native_flow || !r.t81_native_result_str.empty() ||
          !r.t81_native_note.empty()) &&
...
std::string BuildAnalysis(const BenchmarkResult& r) {
  ...
  if (!r.throughput_ratio_computed) {
    ...
    } else if (T81ClassicSemanticallyUnavoidable(r) && !(r.has_t81_flow || r.has_t81_native_flow)) {
      oss << "Throughput ratio not computed (semantic)";
    } else if (IsBelowResolutionMarker(r.t81_result_str) ||
  ...
  } else if (IsBelowResolutionMarker(r.t81_latency_str) ||
  ...
  if (r.native_throughput_ratio_computed || r.native_latency_speedup_computed) {
    if (r.native_throughput_ratio_computed) {
      oss << "; native throughput ratio " << FormatRatio(r.native_throughput_ratio_val)
          << " (T81 native/Binary)";
    }
    if (r.native_latency_speedup_computed) {
      oss << "; native iteration time ratio " << FormatRatio(r.native_latency_speedup_val)
          << " (Binary/T81 native)";
    }
  }
  return oss.str();
...
static bool HasInconsistentCounters(const ::benchmark::BenchmarkReporter::Run& run,
  ...
  if (work_it->second > 0.0 && std::isfinite(static_cast<double>(work_it->second))) return false;
  const double items_per_second = items_it->second;
  const double work_per_iter = work_it->second;
  if (items_per_second <= 0.0 || work_per_iter <= 0.0 || !std::isfinite(items_per_second) ||
      !std::isfinite(work_per_iter)) {
    return false;
  }
  ...
  void ReportRuns(const std::vector<Run>& reports) override {
    ...
    for (const auto& run : reports) {
      ...
      const std::string skip_reason =
          run.skip_message.empty() ? "no counters emitted" : run.skip_message;
      if (skipped) {
        if (is_t81_classic) {
      ...
      bool has_work = work_it != run.counters.end() && work_counter_value > 0.0 &&
                      std::isfinite(work_counter_value);
      double work_per_iter = has_work ? work_counter_value : 0.0;
      bool inferred_work = false;
      if (!has_work && items_per_second > 0.0 && latency > 0.0 && std::isfinite(items_per_second) &&
      ...
      if (!has_work && bandwidth > 0.0 && latency > 0.0 && std::isfinite(bandwidth) &&
          std::isfinite(latency)) {
      ...
      if (effective_label.empty() && has_work) {
        ...
        if (inferred_work) {
          label << "work: inferred units/iter=" << std::fixed << std::setprecision(0)
                << work_per_iter;
        } else {
...
void GenerateMarkdownReport() {
  ...
  auto ResolveLatencySpeedupCell = [&](const BenchmarkResult& r) -> std::string {
    ...
    if (IsBelowResolutionMarker(r.t81_latency_str) ||
        IsBelowResolutionMarker(r.binary_latency_str)) {
      return "below timer resolution";
    }
    if (r.t81_classic_skipped || r.binary_skipped) {
  ...
  auto ResolveNativeThroughputRatioCell = [&](const BenchmarkResult& r) -> std::string {
    ...
    if (IsBelowResolutionMarker(r.t81_native_result_str) ||
        IsBelowResolutionMarker(r.binary_result_str)) {
      return "below timer resolution";
    }
  ...
  auto ResolveNativeLatencySpeedupCell = [&](const BenchmarkResult& r) -> std::string {
    ...
    if (IsBelowResolutionMarker(r.t81_native_latency_str) ||
        IsBelowResolutionMarker(r.binary_latency_str)) {
      return "below timer resolution";
    }
    if (r.t81_native_skipped || r.binary_skipped) {
  ...
  for (auto& [name, r] : final_results) {
    ...
    if (r.t81_native_work_defined && r.binary_work_defined && r.binary_work_per_iter > 0.0) {
      const double rel =
          std::abs(r.t81_native_work_per_iter - r.binary_work_per_iter) / r.binary_work_per_iter;
      if (rel > 0.05) {
        r.t81_native_inconsistent = true;
        r.binary_inconsistent = true;
      }
    ...
    bool throughput_ratio_ready =
        r.has_t81_flow && r.has_binary_flow && r.binary_result_val > 0.0 && r.t81_result_val > 0.0;
    throughput_ratio_ready = throughput_ratio_ready && !IsBelowResolutionMarker(r.t81_result_str) &&
                             !IsBelowResolutionMarker(r.binary_result_str);
    double throughput_ratio = 0.0;
    if (throughput_ratio_ready) {
      throughput_ratio = r.t81_result_val / r.binary_result_val;
    } else if (r.has_t81_flow && r.has_binary_flow && r.binary_bandwidth_val > 0.0 &&
               r.t81_bandwidth_val > 0.0) {
    ...
    if (r.has_t81_native_flow && r.has_binary_flow && r.t81_native_result_val > 0.0 &&
        r.binary_result_val > 0.0 && !IsBelowResolutionMarker(r.t81_native_result_str) &&
        !IsBelowResolutionMarker(r.binary_result_str)) {
  ...
  if (verbose_console) {
    std::cout << std::left << std::setw(25) << "Benchmark" << std::setw(20) << "T81 Result"
              << std::setw(16) << "T81 Iter Time" << std::setw(20) << "Binary Result"
              << std::setw(16) << "Binary Iter" << std::setw(18) << "Thru Ratio" << std::setw(18)
              << "Lat Speedup" << std::setw(18) << "NThru Ratio" << std::setw(18) << "NLat Speedup"
              << std::setw(25) << "T81 Advantage" << "Notes\n";
    ...
    for (auto const& [name, r] : final_results) {
      ...
      std::cout << std::left << std::setw(25) << r.name << std::setw(20)
                << DisplayValue(t81_result_cell) << std::setw(16) << DisplayValue(t81_latency_cell)
                << std::setw(20) << DisplayValue(binary_result_cell) << std::setw(16)
                << DisplayValue(binary_latency_cell) << std::setw(18)
                << DisplayValue(throughput_ratio_cell) << std::setw(18)
                << DisplayValue(latency_speedup_cell) << std::setw(18)
                << DisplayValue(native_throughput_ratio_cell) << std::setw(18)
                << DisplayValue(native_latency_speedup_cell) << std::setw(25)
                << DisplayValue(advantage_display) << DisplayValue(notes_display) << "\n";
    }
  } else {
    std::cout
        << "Benchmark console table suppressed (set T81_BENCHMARK_VERBOSE_CONSOLE=1 to enable).\n";
  }
  ...
  auto CategoryFor = [&](const std::string& name) -> std::string {
    ...
    if (NameContains(name, "T81LangCompile") || NameContains(name, "Lexer"))
      return "Compiler & Language";
    if (NameContains(name, "Limb") || NameContains(name, "Add_") || NameContains(name, "Arith") ||
        NameContains(name, "overflow") || NameContains(name, "Roundtrip") ||
        NameContains(name, "Negation")) {
  ...
  auto FairnessFor = [&](const BenchmarkResult& r,
                         const std::string& notes_display) -> std::string {
  ...
  auto category_counts_for = [](const std::vector<UserRow>& source) {
    ...
    for (const auto& row : source) {
      ...
      counts[0] += 1;
      if (row.verdict == "T81 wins")
        counts[1] += 1;
      else if (row.verdict == "Binary wins")
        counts[2] += 1;
      else
        counts[3] += 1;
    }
  ...
  auto is_product_relevant = [](const UserRow& row) {
    return NameContains(row.name, "Llama") || NameContains(row.name, "CanonFS") ||
           row.name == "BM_T81LangCompile" || row.name == "BM_Lexer_AllTokens" ||
  ...
  auto write_header_and_metadata = [&](std::ofstream& out, const std::string& title) {
    ...
    out << "- Native benchmark mode: SIMD acceleration may fall back to scalar when unsupported by "
           "target/features.\n";
    out << "- CanonFS in-memory binary read baseline is pinned to `baseline=map-read` for "
           "cross-run comparability.\n";
    out << "- CanonFS persistent rows are labeled `comparison=pipeline-advantage` (systems-path "
           "comparison, not strict microkernel fairness).\n\n";
  };

  auto write_decision_tables = [&](std::ofstream& out, const std::vector<UserRow>& report_rows,
                                   const std::map<std::string, std::array<int, 4>>& category_counts,
                                   size_t top_n, bool include_full_appendix,
    ...
    out << "- Comparable benchmark rows (ratio computed): " << report_rows.size() << " / "
        << rows.size() << "\n";
    out << "- Verdict split (comparable rows): T81 wins " << report_t81_wins << ", Binary wins "
        << report_binary_wins << ", Comparable " << report_ties << "\n";
    out << "- Data quality: missing baseline " << missing_baseline << ", semantic mismatch "
        << semantic_mismatch << ", defect rows " << defect_rows << "\n";
    if (!report_best_name.empty()) {
      out << "- Largest T81 advantage: `" << report_best_name << "` ("
          << FormatRatio(report_best_t81_ratio) << ")\n";
    }
    if (!report_worst_name.empty() && report_best_binary_ratio < 1.0) {
      out << "- Largest binary advantage: `" << report_worst_name << "` ("
          << FormatRatio(report_best_binary_ratio) << ")\n";
    }
    out << "\n";
    ...
    for (const auto& [category, counts] : category_counts) {
      ...
      out << "| " << MdCell(category) << " | " << counts[0] << " | " << counts[1] << " | "
          << counts[2] << " | " << counts[3] << " | " << recommendation << " |\n";
    }
    out << "\n";

    out << "## Top Signal Rows\n\n";
    out << "| Benchmark | Category | T81 Iteration | Binary Iteration | Throughput Ratio | Verdict "
           "| Fairness | Confidence |\n";
    out << "|---|---|---|---|---:|---|---|---|\n";
    const size_t max_rows = std::min(top_n, report_rows.size());
    for (size_t i = 0; i < max_rows; ++i) {
      const auto& row = report_rows[i];
      out << "| " << MdCell(row.name) << " | " << MdCell(row.category) << " | "
          << MdCell(row.t81_iter) << " | " << MdCell(row.binary_iter) << " | " << MdCell(row.ratio)
          << " | " << MdCell(row.verdict) << " | " << MdCell(row.fairness) << " | "
    ...
    for (const auto& [category, counts] : category_counts) {
      ...
      out << "### " << MdCell(category) << "\n\n";
      out << "| Benchmark | T81 Iteration | Binary Iteration | Throughput Ratio | Verdict | "
             "Fairness |\n";
      out << "|---|---|---|---:|---|---|\n";
      int emitted = 0;
      for (const auto& row : report_rows) {
        if (row.category != category) continue;
        out << "| " << MdCell(row.name) << " | " << MdCell(row.t81_iter) << " | "
            << MdCell(row.binary_iter) << " | " << MdCell(row.ratio) << " | " << MdCell(row.verdict)
            << " | " << MdCell(row.fairness) << " |\n";
        emitted++;
        if (emitted >= 8) break;
      }
    ...
    if (include_full_appendix) {
      ...
      out << "| Benchmark | Category | T81 Result | T81 Iteration | Binary Result | Binary "
             "Iteration | Throughput Ratio | Verdict | Fairness | Confidence | Notes |\n";
      out << "|---|---|---|---|---|---|---:|---|---|---|---|\n";
      ...
      for (const auto& row : sorted_rows) {
        out << "| " << MdCell(row.name) << " | " << MdCell(row.category) << " | "
            << MdCell(row.t81_result) << " | " << MdCell(row.t81_iter) << " | "
            << MdCell(row.binary_result) << " | " << MdCell(row.binary_iter) << " | "
            << MdCell(row.ratio) << " | " << MdCell(row.verdict) << " | " << MdCell(row.fairness)
            << " | " << MdCell(row.confidence) << " | " << MdCell(row.notes) << " |\n";
      }
    }
  };
  ...
  if (!write_reports) {
    std::cout
        << "Skipping benchmark report generation (set T81_BENCHMARK_WRITE_REPORTS=1 to enable)\n";
    return;
  }
