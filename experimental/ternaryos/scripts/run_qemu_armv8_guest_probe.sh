...
/bin/rm -f "$serial_log" "$pid_file" "$summary_file" "$boot_report_copy" "$startup_status_copy" "$startup_shell_copy" "$startup_session_copy"
...
if [[ "$efi_seen" -ne 1 || "$boot_report_seen" -ne 1 || "$startup_status_seen" -ne 1 || "$startup_shell_seen" -ne 1 || "$startup_session_seen" -ne 1 || "$boot_banner_seen" -ne 1 ]]; then
  echo "QEMU ARMv8 guest probe did not observe the staged BOOTAA64.EFI marker, startup status, startup shell, startup session, boot report, and serial banner" >&2
