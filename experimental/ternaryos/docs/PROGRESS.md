...
- The local QEMU developer lane now carries a Phase 4 guest artifact too: `startup-phase4.txt` is derived from the real guest bootstrap path and proves AHCI-backed CanonStore recovery for three persisted objects across two guest cycles, successful torn-header recovery for those same objects, one VMSVGA present, and one E1000 send/receive round-trip under the staged ARM guest.
