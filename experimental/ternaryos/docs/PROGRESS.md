...
- The local QEMU developer lane now carries a Phase 4 guest artifact too: `startup-phase4.txt` is derived from the real guest bootstrap path and proves AHCI-backed CanonStore recovery for 20 persisted objects across two guest cycles, active overflow metadata beyond the 17-entry root header, successful torn-header recovery for those same objects, two distinct VMSVGA presents from the staged framebuffer path, and a three-frame E1000 send/inject/receive workload under the staged ARM guest.
