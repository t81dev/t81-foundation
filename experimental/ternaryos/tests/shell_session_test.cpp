...
static void test_scripted_shell_session() {
  ...
  check(state->available_commands.size() == 11, "eleven builtins are exposed");
...
static void test_typed_shell_commands() {
  ...
  check(state_before_clear.command_records.size() == 13, "interactive session records thirteen commands");
  ...
  check(state_before_clear.command_records[3].result.starts_with("session refs 1"),
  ...
  check(state_before_clear.command_records[4].result.starts_with("store refs 1"),
  ...
  check(state_before_clear.command_records[5].result == "store get typed shell payload",
        "store get decodes stored payload");
  check(state_before_clear.command_records[6].result.starts_with("history durable " + stored_ref),
        "history show durable exposes the durable anchor ref");
  check(state_before_clear.command_records[6].result.find("typed shell payload") != std::string::npos,
        "history show durable exposes the durable payload");
  check(state_before_clear.command_records[7].result == "store rm ok " + stored_ref,
        "store rm reports durable removal");
  check(state_before_clear.command_records[8].result == "store get missing",
        "removed ref is no longer readable");
  check(state_before_clear.command_records[9].result == "reboot history missing",
        "history reports missing durable anchor after removal");
  check(state_before_clear.command_records[10].result == "history durable missing",
        "history show durable reports missing anchor after removal");
  check(state_before_clear.command_records[11].result == "parse error: unmatched quote",
        "unmatched quote is surfaced as a parse error");
  check(state_before_clear.command_records[12].result == "unknown command", "unknown command is surfaced");
  check(state_before_clear.recovered_entries == 0, "interactive history refresh tracks current recovered_entries");
  check(state_before_clear.session_command_count == 13,
  ...
  check(state_after_clear.command_records.size() == 4, "clear resets transcript to the new session window");
  ...
  check(state_after_clear.command_records[2].result == "session refs 0",
        "post-clear session refs shows no durable refs");
  check(state_after_clear.command_records[3].result == "history durable missing",
        "post-clear history show durable still sees missing durable anchor");
  check(state_after_clear.session_command_count == 4,
