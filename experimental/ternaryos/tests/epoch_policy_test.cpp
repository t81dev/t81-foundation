...
static bool counting_gate_fn(const TaskDescriptor& task,
                              const t81::tisc::Program& /*prog*/,
                              void* user_data) noexcept {
  auto* ctx = static_cast<GateContext*>(user_data);
  ++ctx->calls;
  if (ctx->deny_task_seq >= 0 &&
      static_cast<int>(task.task_seq) == ctx->deny_task_seq) {
    return false;  // deny
  }
  return true;  // allow
}
...
static void test_policy_denial_aborts_epoch(KernelRuntimeState& state) {
  ...
  check(!result.ok(),
        "[DPE-03-06-01] result.ok() == false");

  // Audit log must contain exactly one new EpochAbortedPolicyFault record.
  check(state.audit_log.size() == audit_log_before + 1,
        "[DPE-03-06-02] audit_log grew by 1");
  const bool audit_kind_correct =
      !state.audit_log.empty() &&
      state.audit_log.back().kind == KernelAuditEventKind::EpochAbortedPolicyFault;
  check(audit_kind_correct,
        "[DPE-03-06-02] audit_log.back().kind == EpochAbortedPolicyFault");

  // last_audit_event must reflect the policy fault.
  check(state.last_audit_event.has_value(),
        "[DPE-03-06-02] last_audit_event is set");
...
static void test_first_task_denial_short_circuits(KernelRuntimeState& state) {
  ...
  check(ctx.calls == 1,
        "[DPE-03-06-06] gate called exactly once (first-task short-circuit)");
}
