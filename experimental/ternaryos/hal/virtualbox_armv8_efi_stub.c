...
static EFI_STATUS write_startup_store(EFI_HANDLE image_handle,
                                      EFI_SYSTEM_TABLE* system_table) {
  EFI_FILE_PROTOCOL* root = 0;
  EFI_STATUS status = open_root_volume(image_handle, system_table, &root);
  if (status != EFI_SUCCESS) {
  ...
  status = write_root_file(root,
                           kStartupStorePath,
                           kGeneratedStartupStore,
                           ascii_length(kGeneratedStartupStore));
  root->Close(root);
  return status;
}
...
static EFI_STATUS write_startup_report(EFI_HANDLE image_handle,
                                       EFI_SYSTEM_TABLE* system_table) {
  EFI_FILE_PROTOCOL* root = 0;
  EFI_STATUS status = open_root_volume(image_handle, system_table, &root);
  if (status != EFI_SUCCESS) {
  ...
  status = write_root_file(root,
                           kStartupReportPath,
                           kGeneratedStartupReport,
                           ascii_length(kGeneratedStartupReport));
  root->Close(root);
  return status;
}
