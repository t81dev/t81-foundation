...
| `shell_startup_snapshot.cpp` | Build-time Axion shell snapshot generator: runs the real Phase 5 shell backend, emits compact startup-shell, startup-session, startup-history, startup-store, and startup-ref artifacts, and produces the generated headers embedded by the ARM EFI developer-lane stub | — |
