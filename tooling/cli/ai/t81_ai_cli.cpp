...
bool parse_named_args(int argc, char* argv[], int start, Options& opts, std::vector<std::string>& positional,
                      std::string& error) {
  for (int i = start; i < argc; ++i) {
    ...
    if (arg == "--model-file") {
      ...
      opts.model_file = fs::absolute(argv[++i]);
    } else if (arg == "--out") {
      if (i + 1 >= argc) {
        error = "missing value for --out";
        return false;
      }
      opts.out = fs::absolute(argv[++i]);
    } else if (arg == "--model") {
      if (i + 1 >= argc) {
...
int cmd_quantization_inspect(const Options& opts) {
  ...
  return emit_or_write(
      make_runtime_payload("t81.ai.quantization-inspect.v1", model_id, model_file, sel, extra.str()),
      opts.out);
}
...
int cmd_policy_test(const Options& opts) {
  ...
  const std::string reason_code = allow ? std::string(kPolicyAllowReason)
                                        : "AI_POLICY_DENY_UNKNOWN_EVENT_TYPE";
  std::ostringstream out;
  out << "{\n"
...
int cmd_observability_trace(const fs::path& out_path) {
  std::string trace_seed = std::string("model_load|allow|") + std::string(kPolicyAllowReason);
  const std::string trace_hash = sha3_hex_text(trace_seed);
  std::ostringstream payload;
  payload << "{\n"
...
int main(int argc, char* argv[]) {
  ...
  if (command == "backend") {
    ...
    if (sub == "select") {
      if (!parse_named_args(argc, argv, 3, opts, positional, error)) {
        std::cerr << "error: " << error << "\n";
        return 1;
      }
    ...
    if (argc >= 4 && std::string(argv[2]) == "inspect") {
      return cmd_model_inspect(fs::path(argv[3]));
    }
  } else if (command == "verify") {
    if (argc >= 4 && std::string(argv[2]) == "determinism") {
      return cmd_verify_file(fs::path(argv[3]));
    }
    if (!parse_named_args(argc, argv, 2, opts, positional, error)) {
      std::cerr << "error: " << error << "\n";
      return 1;
    }
    ...
    if (argc >= 3 && std::string(argv[2]) == "run") {
      if (!parse_named_args(argc, argv, 4, opts, positional, error)) {
        std::cerr << "error: " << error << "\n";
        return 1;
      }
      return cmd_workflow_run(argc >= 4 ? argv[3] : "", opts);
    }
    if (argc >= 4 && std::string(argv[2]) == "replay") {
      return cmd_workflow_replay(fs::path(argv[3]));
    }
    if (argc >= 4 && std::string(argv[2]) == "report") {
      return cmd_workflow_report(fs::path(argv[3]));
    }
  } else if (command == "observability") {
    if (argc >= 4 && std::string(argv[2]) == "trace") {
      return cmd_observability_trace(fs::absolute(fs::path(argv[3])));
    }
